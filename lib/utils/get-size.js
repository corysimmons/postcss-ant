...
export default (node: {
  ...
  switch (node.value) {
    ...
    case `${opts.namespace}sizes`:
      ...
      if (sizes.find(size => /,/.test(size))) {
        errHandler(decl, `You can't have a comma in sizes().`, `Try removing the comma. If you're trying to use size sets (which only work with generate-grid), use columns() and/or rows() instead of sizes().`)
      }

      results.push(calcHell(sizes, opts))
      break
    ...
    case `${opts.namespace}rows`:
      ...
      sizeSets.map(set => {
        ...
        for (let i = 1; i <= sizes.length; i++) {
          ...
          opts.pluck = i
          setResults.push(calcHell(sizes, opts))
        }
