  ...
  return (css: {
    ...
    css.walkDecls((decl: {
      ...
      const optsParsed: {
        ...
        const optsRegexp = new RegExp(`${opts.namespace}(?=gutters?|rounders?|support|pluck|bump|technique|children)`)
        if (node.type === 'function' && optsRegexp.test(node.value)) {
          ...
          switch (node.value) {
            ...
            case `${opts.namespace}gutters`:
              opts.gutters = postcss.list.comma(valueParser.stringify(node.nodes))
              break

            case `${opts.namespace}rounder`:
            case `${opts.namespace}rounders`:
              opts.rounders = postcss.list.comma(valueParser.stringify(node.nodes))
              break

            case `${opts.namespace}support`:
              opts.support = valueParser.stringify(node.nodes)
              break

            case `${opts.namespace}pluck`:
              opts.pluck = Number(valueParser.stringify(node.nodes))
              break
            ...
            case `${opts.namespace}technique`:
              opts.technique = valueParser.stringify(node.nodes)
              break

            case `${opts.namespace}children`:
              opts.children = valueParser.stringify(node.nodes)
              break
      ...
      if (foundColumns) {
        valueParser(decl.value).walk((node: {
          ...
          const rowsRegexp = new RegExp(`${opts.namespace}(?=rows)`)
          ...
          switch (node.value) {
            ...
            case `${opts.namespace}rows`:
              ...
              if (ggRegexp.test(decl.prop)) {
                generateGrid(node, opts, node.value, decl, firstColumnSetLength, foundColumnsAndRows, prevSourceIndex)
              }
              break
